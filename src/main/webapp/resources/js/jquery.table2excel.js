	...
	Plugin.prototype = {
								...
								$(o)
										...
										.each(
												function(i, p) {
													...
													// Preserve background and
													// text colors on the row
													if (e.settings.preserveColors) {
														compStyle = getComputedStyle(p);
														additionalStyles += (compStyle
																&& compStyle.backgroundColor ? "background-color: "
		...
		tableToExcel : function(table, name, sheetName) {
			...
			if ($.isArray(table)) {
				Object.keys(table).forEach(
						function(i) {
							...
							fullTemplate += e.template.sheet.head + sheetName
									+ i + e.template.sheet.tail;
						});
			}
			...
			if ($.isArray(table)) {
				Object.keys(table).forEach(
						function(i) {
							fullTemplate += e.template.table.head + "{table"
									+ i + "}" + e.template.table.tail;
						});
			}
			...
			var isIE = navigator.appVersion.indexOf("MSIE 10") !== -1
					|| (navigator.userAgent.indexOf("Trident") !== -1 && navigator.userAgent
							.indexOf("rv:11") !== -1); // this works with IE10
														// and IE11 both :)
			// if (typeof msie !== "undefined" && msie > 0 ||
			// !!navigator.userAgent.match(/Trident.*rv\:11\./)) // this works
			// ONLY with IE 11!!!
			if (isIE) {
				if (typeof Blob !== "undefined") {
					// use blobs if we can
					fullTemplate = e.format(fullTemplate, e.ctx); // with
																	// this,
																	// works
																	// with IE
